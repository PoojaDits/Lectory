    ...
    <div className="space-y-6 mt-[65px] animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-secondary-100 pb-6">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-full bg-primary-700 px-5 py-2.5 text-sm font-black text-white shadow-md hover:bg-primary-800 transition"
        >
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-[2.5rem] border border-dashed border-secondary-200 bg-white/70 p-16 text-center">
          ...
          <h3 className="text-lg font-black text-secondary-900">No listings yet</h3>
          ...
          <button
            type="button"
            onClick={() => setShowCreate(true)}
            className="mt-5 rounded-full bg-primary-700 px-6 py-3 text-sm font-black text-white hover:bg-primary-800 transition"
          >
            Create a Listing
          </button>
        </div>
...
function ListingCard({
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-secondary-200 bg-white p-5 shadow-sm transition hover:shadow-md">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="font-black text-secondary-900 text-sm line-clamp-2">
      ...
      {isEditing ? (
        <div className="mt-4 space-y-3">
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-secondary-200 bg-secondary-50 py-2 pl-8 pr-2 text-sm font-bold outline-none focus:border-amber-500 focus:ring-4 focus:ring-primary-100"
                />
              </div>
            </div>
            <div>
          ...
          <div className="flex gap-2 pt-1">
            <button
              ...
              disabled={isBusy}
              className="flex-1 rounded-full bg-primary-700 py-2 text-xs font-black text-white hover:bg-primary-800 transition disabled:opacity-50"
            >
              Save
            </button>
            <button
              ...
              disabled={isBusy}
              className="flex-1 rounded-full border border-secondary-200 bg-white py-2 text-xs font-black text-secondary-700 hover:bg-secondary-50 transition"
            >
              Cancel
            </button>
          </div>
        ...
        <div className="mt-4 space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-black text-secondary-900">{formatCurrency(listing.price)}</span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-500">Stock</span>
            <span className={`font-black ${listing.stock > 0 ? "text-primary-700" : "text-rose-600"}`}>
          ...
          <div className="flex gap-2 pt-3">
            <button
              type="button"
              onClick={onEdit}
              className="flex-1 inline-flex items-center justify-center gap-1 rounded-full border border-secondary-200 bg-white py-2 text-xs font-black text-secondary-700 hover:bg-secondary-50 transition"
            >
      ...
      {book?.id && (
        <Link
          to={`/books/${book.id}`}
          className="mt-4 text-center text-xs font-bold text-primary-700 hover:text-primary-800"
        >
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-extrabold text-secondary-900">Create Listing</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-full p-1 text-slate-400 hover:bg-secondary-100"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div className="mt-4 flex items-center justify-end gap-2 border-t border-secondary-100 pt-4">
            ...
            <button
              type="submit"
              disabled={isPending}
              className="inline-flex items-center gap-2 rounded-full bg-primary-700 px-5 py-2 text-sm font-bold text-white hover:bg-primary-800 transition disabled:opacity-50"
            >
              {isPending ? (
                <>
