...
export default function StoreDetailsPage({
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-primary-50 via-white to-orange-50 pb-20 pt-24">
      <div className="mx-auto w-full px-6 sm:px-12 md:px-16 lg:px-24 xl:px-32">
        {/* Back nav */}
        <button
          type="button"
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <Link
              to="/stores"
              className="mt-6 inline-flex items-center gap-2 rounded-full bg-primary-900 px-5 py-2.5 text-sm font-bold text-white"
            >
              Browse all stores
            </Link>
          </div>
          ...
          <>
            ...
            <section className="mt-10">
              <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h2 className="text-2xl font-black tracking-tight text-secondary-900">
                    Books in this store
                  </h2>
              ...
              {books.length === 0 ? (
                <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-secondary-200 bg-white py-16 text-center">
                  <div className="rounded-full bg-secondary-100 p-6">
...
function StoreHeader({
  ...
  return (
    <header className="overflow-hidden rounded-3xl border border-secondary-100 bg-white shadow-sm">
      {/* Banner */}
      <div className="relative h-40 w-full bg-gradient-to-br from-primary-600 via-primary-700 to-primary-900 sm:h-48">
        {store.logo ? (
          <LazyImage
            src={store.logo}
      ...
      <div className="px-6 pb-6 sm:px-8 mt-[65px]">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex gap-3 pb-1">
            <div className="rounded-xl bg-primary-50 px-4 py-2 text-center">
              <p className="text-lg font-black text-primary-900">{bookCount}</p>
        ...
        {/* Description */}
        {store.description && (
          <p className="mt-5 max-w-3xl text-sm leading-relaxed text-secondary-600">
            {store.description}
          </p>
        )}

        {/* Meta row */}
        <div className="mt-5 flex flex-wrap gap-x-6 gap-y-2 border-t border-secondary-100 pt-5 text-sm text-secondary-600">
...
function StoreHeaderSkeleton() {
  return (
    <div className="overflow-hidden rounded-3xl border border-secondary-100 bg-white shadow-sm">
      <div className="h-48 w-full animate-pulse bg-secondary-200" />
      <div className="space-y-4 px-8 pb-8">
        <div className="h-8 w-1/2 animate-pulse rounded bg-secondary-200" />
...
function StoreBookCard({
  ...
  return (
    <Link
      to={`/books/${book.id}`}
      className="group flex h-full w-full max-w-[240px] flex-col overflow-hidden rounded-2xl border border-secondary-100 bg-white shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl"
    >
      <div className="relative aspect-[3/4] w-full overflow-hidden bg-secondary-50 p-3">
        {book.coverImage ? (
          <LazyImage
            src={book.coverImage}
            alt={book.title}
          ...
          <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-primary-100 to-orange-100 p-3">
            <span className="text-center text-sm font-bold text-primary-800 line-clamp-3">
              {book.title}
            </span>
          </div>
        )}
        ...
        <div className="absolute right-2 top-2">
          {outOfStock ? (
            <span className="rounded-full bg-secondary-800 px-2 py-1 text-[10px] font-bold text-white shadow-sm">
              Out of stock
            </span>
          ) : (
            <span className="rounded-full bg-white px-2 py-1 text-[10px] font-bold text-primary-900 shadow-sm">
